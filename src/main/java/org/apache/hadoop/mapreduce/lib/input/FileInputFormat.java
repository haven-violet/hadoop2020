...
public abstract class FileInputFormat<K, V> extends InputFormat<K, V> {
     ...
     *
     * @param job the job context sss 这里是分片的开始代码
     * @throws IOException
     */
    public List<InputSplit> getSplits(JobContext job) throws IOException {
        ...
        // generate splits sss InputSplit代表一个切片类
        List<InputSplit> splits = new ArrayList<InputSplit>();
        //sss 得到所有输入文件列表
        List<FileStatus> files = listStatus(job);
        //sss 循环取出每一个文件， 根据每个文件来切片,而不是整个文件夹来切
        for (FileStatus file : files) {
            Path path = file.getPath();
            long length = file.getLen(); //sss 获取文件大小及位置
            if (length != 0) {
                BlockLocation[] blkLocations;
                if (file instanceof LocatedFileStatus) {//sss 找到当前切片的块的位置
                    blkLocations = ((LocatedFileStatus) file).getBlockLocations();
                } else {
                    FileSystem fs = path.getFileSystem(job.getConfiguration());
                    blkLocations = fs.getFileBlockLocations(file, 0, length);
                }
                //sss 判断文件是否可以分片（压缩格式有的可以进行分片，有的不可以，）
                if (isSplitable(job, path)) {
                    long blockSize = file.getBlockSize();// sss 默认1.x中块的大小64,2.x是128,3.x默认256,但是是本地模式下块大小为32M

                    //sss 得到实际切片的大小,如果是本地文件,默认切片是32M,经典面试题,怎么调整片的大小,怎么计算片的大小
                    long splitSize = computeSplitSize(blockSize, minSize, maxSize);
                    //sss byteRemaining表示当前还没有切片的文件剩余大小
                    long bytesRemaining = length;
                    //sss  SPLIT_SLOP=1.1 只有文件长达大于块的1.1倍才会切片
                    while (((double) bytesRemaining) / splitSize > SPLIT_SLOP) {
                        //sss 封装一个分片信息(包含文件的路径，分片的起始偏移量，要处理的大小，分片包含的块的信息，分片中包含的块存在哪儿些机器上)剩余文件的大小/分片大小<=1.1且
                        int blkIndex = getBlockIndex(blkLocations, length - bytesRemaining);
                        //sss makeSplit进行切片操作,返回值是一个切片,并且加入到切片列表中
                        splits.add(makeSplit(path, length - bytesRemaining, splitSize,
                                blkLocations[blkIndex].getHosts(),
                                blkLocations[blkIndex].getCachedHosts()));
                        bytesRemaining -= splitSize;
                    }
                    //sss 不等于0时，封装一个分片信息(包含文件的路径，分片的起始偏移量，要处理的大小，分片包含的块的信息,分片中包含的块存在哪儿些机器上)
                    if (bytesRemaining != 0) {//sss 把最后剩余部分单独放在一个切片中,譬如文件50M 切片32M 那么现在切片大小是18M
                        int blkIndex = getBlockIndex(blkLocations, length - bytesRemaining);
            ...
            } else {//sss 如果文大小为空,默认就创建一个空的hosts文件数组和空长度的文件
...
//sss 通过块的大小,片的最小值,最大值来获得一个最后的切片大小,默认块的大小不变,所以一般通过调整minSize,maxSize来确定片的大小
