...
class JobSubmitter {
  ...
  throws ClassNotFoundException, InterruptedException, IOException {
    ...
    try {
      ...
      //sss mr 在本地提交Job的文件名为:file:/tmp/hadoop-samli/mapred/staging/samli1570790902/.staging/job_local1570790902_0001/job.xml
      Path submitJobFile = JobSubmissionFiles.getJobConfPath(submitJobDir);
      
      // Create the splits for the job
      LOG.debug("Creating splits at " + jtFs.makeQualified(submitJobDir));
      int maps = writeSplits(job, submitJobDir);//sss mr 在JobSubmitter中给job任务文件切片
      ...
      //sss mr 在JobSubmitter中真正submit任务的地方
  ...
  int writeNewSplits(JobContext job, Path jobSubmitDir) throws IOException,
    ...
    //sss 在JobSubmitter中就FileInputFormat进行切片,并且返回一个切片列表
  ...
  private int writeSplits(JobContext job,
