 ...
 private <T> T getSplitDetails(Path file, long offset) 
  ...
  private class NewOutputCollector<K,V>
    ...
    NewOutputCollector(JobContext jobContext,
      ...
      if (partitions > 1) {//sss 如果partitions>1,才会找我们自定义的分区类
        partitioner = (org.apache.hadoop.mapreduce.Partitioner<K,V>)
          ReflectionUtils.newInstance(jobContext.getPartitionerClass(), job);
      } else {  //sss 如果partitions(numReduceTasks)=1 那么直接调用匿名分区类,返回的分区号是1-1=0
    ...
    @Override // sss 在写数据的时候除了k,v把分区数也写入进去了
  ...
  public static class MapOutputBuffer<K extends Object, V extends Object>
     ...
     * storage to store one METADATA. sss mr 在从MapTask往收集器中写数据时,除了写入k,v外,
     * sss 把当前k,v所在的分区号也写入,保证写在环形缓冲区中的每对kv都知道自己所在分区
     */
