...
public class LocalJobRunner implements ClientProtocol {
  ...
  private class Job extends Thread implements TaskUmbilicalProtocol {
    ...
    protected class MapTaskRunnable extends RunnableWithThrowable {
      ...
      public void run() {
        try {//sss mr 真正的Job在LocalJobRunner用多线程方式启动的地方
          ...
          try {
            ...
            map.run(localConf, Job.this);//sss mr 在localJobRunner中运行Task的地方
    ...
    private void runTasks(List<RunnableWithThrowable> runnables,
      ...
      for (Runnable r : runnables) {
        service.submit(r);//sss mr 通过线程池里面的submit提交子任务的执行,每个线程对应一个Task
      }
    ...
    @Override
    public void run() {//sss mr 真正的Job在LocalJobRunner用多线程方式启动的地方
  ...
  /**
   * @see ClientProtocol#getSystemDir()
   */
  ...
  /**
   * @see ClientProtocol#getQueueAdmins(String)
   */
  ...
  /**
   * @see ClientProtocol#getStagingAreaDir()
   */
