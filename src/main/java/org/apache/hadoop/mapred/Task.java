...
abstract public class Task implements Writable, Configurable {
  ...
  protected static List<Statistics> getFsStatistics(Path path, Configuration conf) throws IOException {
    List<Statistics> matchedStats = new ArrayList<Statistics>();
  ...
  class FileSystemStatisticUpdater {
    ...
    FileSystemStatisticUpdater(List<Statistics> stats, String scheme) {
      this.stats = stats;
      this.scheme = scheme;
    }
  ...
  private synchronized void updateCounters() {
    Map<String, List<Statistics>> map = new
        HashMap<String, List<Statistics>>();
    for(Statistics stat: FileSystem.getAllStatistics()) {
      String uriScheme = stat.getScheme();
      if (map.containsKey(uriScheme)) {
        List<Statistics> list = map.get(uriScheme);
        list.add(stat);
      } else {
    ...
    for (Map.Entry<String, List<Statistics>> entry: map.entrySet()) {
    ...
    public void combine(RawKeyValueIterator iterator, 
      ...
      org.apache.hadoop.mapreduce.Reducer.Context 
           reducerContext = createReduceContext(reducer, job, taskId,
                                                ...
                                                valueClass);
      reducer.run(reducerContext);//sss 在combine方法中其实还是调了reducer的run方法
    } 
  }
